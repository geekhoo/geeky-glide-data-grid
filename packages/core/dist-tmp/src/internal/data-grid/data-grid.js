...
const DataGrid = (p, forwardedRef) => {
    ...
    const accessibilityTree = useDebouncedMemo(() => {
        ...
        return (_jsxs("table", { role: "grid", "aria-rowcount": rows + 1, "aria-multiselectable": "true", "aria-colcount": mappedColumns.length + colOffset, children: [_jsx("thead", { role: "rowgroup", children: _jsx("tr", { role: "row", "aria-rowindex": 1, children: effectiveCols.map(c => (_jsx("th", { role: "columnheader", "aria-selected": selection.columns.hasIndex(c.sourceIndex), "aria-colindex": c.sourceIndex + 1 + colOffset, tabIndex: -1, onFocus: e => {
                                if (e.target === focusRef.current)
                                    return;
                                return onCellFocused?.([c.sourceIndex, -1]);
                            }, children: c.title }, c.sourceIndex))) }) }), _jsx("tbody", { role: "rowgroup", children: visibleRows.map(row => (_jsx("tr", { role: "row", "aria-selected": selection.rows.hasIndex(row), "aria-rowindex": row + 2, children: effectiveCols.map(c => {
                            ...
                            return (_jsx("td", { role: "gridcell", "aria-colindex": col + 1 + colOffset, "aria-selected": selected, "aria-readonly": isInnerOnlyCell(cellContent) || !isReadWriteCell(cellContent), id: id, "data-testid": id, onClick: () => {
                                ...
                                }, ref: focused ? focusElement : undefined, tabIndex: -1, children: getRowData(cellContent, getCellRenderer) }, key));
                        }) }, row))) })] }, "access-tree"));
    }, [
        width,
        mappedColumns,
    ...
    const stickyShadow = React.useMemo(() => {
        ...
        return (_jsxs(_Fragment, { children: [opacityX > 0 && _jsx("div", { id: "shadow-x", style: styleX }), opacityY > 0 && _jsx("div", { id: "shadow-y", style: styleY })] }));
    ...
    return (_jsxs(_Fragment, { children: [_jsx("canvas", { "data-testid": "data-grid-canvas", tabIndex: 0, onKeyDown: onKeyDownImpl, onKeyUp: onKeyUpImpl, onFocus: onCanvasFocused, onBlur: onCanvasBlur, ref: refImpl, style: style, children: accessibilityTree }), _jsx("canvas", { ref: overlayRef, style: overlayStyle }), stickyShadow] }));
};
