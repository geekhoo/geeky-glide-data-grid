...
describe("data-grid-overlay", () => {
    ...
    test("Smoke test bubbles", async () => {
        render(_jsx(BubblesOverlayEditor, { bubbles: ["A", "B"] }));
    });
    test("Smoke test drilldown", async () => {
        render(_jsx(DrilldownOverlayEditor, { drilldowns: [{ text: "A" }, { text: "B" }] }));
    });
    test("Smoke test image overlay", async () => {
        const spy = vi.fn();
        render(_jsx(ImageOverlayEditor, { canWrite: false, onCancel: spy, onChange: spy, urls: ["https://www.google.com"] }));
    });
    test("Smoke test markdown overlay", async () => {
        const spy = vi.fn();
        render(_jsx(MarkdownOverlayEditor, { forceEditMode: false, value: {
                kind: GridCellKind.Markdown,
                allowOverlay: true,
                data: "# Header",
            }, onChange: spy, onFinish: spy, targetRect: { x: 0, y: 0, width: 200, height: 32 } }));
    });
    test("Smoke test number overlay", async () => {
        const spy = vi.fn();
        render(_jsx(NumberOverlayEditor, { highlight: false, onChange: spy, value: 35 }));
    });
    test("Smoke test uri overlay editor", async () => {
        const spy = vi.fn();
        render(_jsx(UriOverlayEditor, { forceEditMode: false, onChange: spy, readonly: false, uri: "https://google.com", preview: "https://google.com" }));
    });
});
