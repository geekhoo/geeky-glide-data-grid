...
import renderer, { formatValueForHTMLInput, } from "../src/cells/date-picker-cell.js";
...
describe("formatValueForHTMLInput", () => {
    ...
    ])("check %p format and %p date leads correct format: {%p}", (format, date, valueForHTML) => {
        expect(formatValueForHTMLInput(format, date)).toStrictEqual(valueForHTML);
    });
    it("throws an error when a weird value is passed", () => {
        expect(() => formatValueForHTMLInput("weird", new Date())).toThrow("Unknown date kind weird");
    });
});
describe("editor", () => {
    afterEach(cleanup);
    function getMockDateCell(props = {}) {
        return {
            ...props,
    ...
    it("renders into the dom with correct value", () => {
        ...
        const result = render(_jsx(Editor, { isHighlighted: false, value: getMockDateCell() }));
    ...
    it.each([["date"], ["time"], ["datetime-local"]])("renders with correct format", (format) => {
        // @ts-ignore
        const Editor = renderer.provideEditor?.(getMockDateCell({ data: { format: format } })
        // @ts-ignore
        ).editor;
        if (Editor === undefined) {
            throw new Error("Editor is invalid");
    ...
    it("renders textarea when readonly is true", () => {
        // @ts-ignore
        const Editor = renderer.provideEditor?.(getMockDateCell({ readonly: true })
        // @ts-ignore
        ).editor;
        if (Editor === undefined) {
            throw new Error("Editor is invalid");
    ...
    it('properly sets date when value is NOT ""', async () => {
        ...
        const result = render(_jsx(Editor, { isHighlighted: false, value: getMockDateCell(), onChange: mockCellOnChange }));
...
describe("onPaste", () => {
    ...
    ])("correctly interprets pasted value %p as %p", (input, expected) => {
        // @ts-ignore
        const { date } = renderer.onPaste(input, {});
        expect(date.toISOString()).toStrictEqual(expected);
    });
    it.each([[""], ["invalid"], ["2020-20-12"], ["2020/20/12"], [undefined], [null], ["2020-12-10-10"]])("correctly returns no value when onPaste is called with invalid value: %p", (input) => {
        // @ts-ignore
        const { date } = renderer.onPaste(input, {});
