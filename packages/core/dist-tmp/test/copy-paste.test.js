...
import { GridCellKind, BooleanIndeterminate } from "../src/index.js";
import { decodeHTML, getCopyBufferContents } from "../src/data-editor/copy-paste.js";
import { expect, describe, test } from "vitest";
function makeCellBuffer(rawValue, formatted = rawValue, format = "string") {
    return {
        rawValue,
        formatted,
        format,
...
describe("copy-paste", () => {
    ...
    test("Simple text cell with multiple spaces", () => {
        ...
        const result = getCopyBufferContents(cells, columnIndexes);
        expect(result.textPlain).toBe("Display  Hello");
        expect(result.textHtml).toContain('<td gdg-raw-value="Hello" gdg-format="string">Display<span> </span><span> </span>Hello</td>');
    });
    ...
    test("Bubble cell encoding", () => {
        ...
        expect(result.textHtml).toContain('<td gdg-format="string-array"><ol><li gdg-raw-value="Bubble1">Bubble1</li><li gdg-raw-value="Bubble2">Bubble2</li></ol></td>');
    });
    ...
    test("format url cell", () => {
        ...
        ], [0]).textPlain).toEqual("https://www.google.com");
    });
    ...
    test("format empty bubble cell with comma", () => {
        ...
        ], [0]).textPlain).toEqual('"foo, bar",baz');
    });
    ...
    test.each([
        ...
        expect(result.textHtml).toContain(`<td gdg-raw-value="${data ?? ""}" gdg-format="boolean">${expectedFormatted}</td>`);
    });
    test("Image cell type", () => {
        ...
        expect(result.textHtml).toContain('<td gdg-format="string-array"><ol><li gdg-raw-value="image1.jpg">image1.jpg</li><li gdg-raw-value="image2.jpg">image2.jpg</li></ol></td>');
    });
    test.each([
        ...
        const result = getCopyBufferContents(cells, columnIndexes);
        expect(result.textPlain).toBe(expectedFormatted);
        expect(result.textHtml).toContain(`<td gdg-raw-value="${data ?? ""}" gdg-format="${format}">${expectedFormatted}</td>`);
    });
...
test("Drilldown cell conversion", () => {
    ...
    expect(result.textHtml).toContain('<td gdg-format="string-array"><ol><li gdg-raw-value="Drill1">Drill1</li><li gdg-raw-value="Drill2">Drill2</li></ol></td>');
});
