...
const Editor = cell => {
    ...
    if (cell.value.readonly) {
        return (_jsx(TextCellEntry, { highlight: true, autoFocus: false, disabled: true, value: displayDate ?? "", onChange: () => undefined }));
    }
    return (_jsx(StyledInputBox, { "data-testid": "date-picker-cell", required: true, type: format, defaultValue: value, min: minValue, max: maxValue, step: step, autoFocus: true, onChange: event => {
