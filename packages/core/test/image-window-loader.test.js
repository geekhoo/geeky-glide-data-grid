...
describe("ImageWindowLoaderImpl", () => {
    ...
    describe("setWindow()", () => {
        ...
        it("should call clearOutOfWindow() if the window or freezeCols changes", () => {
            const spyClearOutOfWindow = vi.spyOn(loader, "clearOutOfWindow"); // Private method, so using 'as any'
            const window1 = {
                x: 10,
                y: 10,
                width: 100,
    ...
    describe("loadOrGetImage()", () => {
        ...
        it("should trigger image loading if the image is not in the cache", () => {
            ...
            const spyLoadImage = vi.spyOn(loader, "loadImage");
            loader.cache = {};
    ...
    describe("loadImage()", () => {
        ...
        it("should add a loading image to the cache", () => {
            // Act: Load an image
            loader.loadImage(url, col, row, key);
            // Assert: Ensure the cache was updated with an entry for the loading image
            expect(loader.cache[key]).toBeDefined();
            expect(loader.cache[key].url).toBe(url);
            expect(loader.cache[key].img).toBeUndefined();
            expect(loader.cache[key].cells).toEqual([packColRowToNumber(col, row)]);
        });
        it("should handle image loading and decoding successfully", async () => {
            // Setup: Spy on `Image` constructor and `decode` method
            const img = new Image();
            img.addEventListener = vi.fn((_kind, cb) => cb());
            vi.useFakeTimers();
            ...
            expect(loader.cache[key].img).toBe(img);
