...
import DataGrid, {} from "../src/internal/data-grid/data-grid.js";
...
describe("data-grid", () => {
    ...
    test("Emits mouse down", () => {
        const spy = vi.fn();
        render(_jsx(DataGrid, { ...basicProps, onMouseDown: spy }));
    ...
    test("Emits mouse up", () => {
        const spy = vi.fn();
        render(_jsx(DataGrid, { ...basicProps, onMouseUp: spy }));
        fireEvent.mouseDown(screen.getByTestId(dataGridCanvasId), {
            clientX: 300, // Col B
    ...
    test("Does not emit mousedown/up over header menu", () => {
        const downSpy = vi.fn();
        const upSpy = vi.fn();
        render(_jsx(DataGrid, { ...basicProps, columns: basicProps.columns.map(c => ({ ...c, hasMenu: true })), onMouseUp: upSpy, onMouseDown: downSpy }));
    ...
    test("Cell hovered", () => {
        const spy = vi.fn();
        render(_jsx(DataGrid, { ...basicProps, onItemHovered: spy }));
        const el = screen.getByTestId(dataGridCanvasId);
    ...
    test("Cell is not hovered when target is not data grid", () => {
        const spy = vi.fn();
        render(_jsxs(_Fragment, { children: [_jsx(DataGrid, { ...basicProps, onItemHovered: spy }), _jsx("div", { "data-testid": "outside-element", style: {
                        position: "absolute",
                        width: "100vh",
                        height: "100vh",
                    } })] }));
    ...
    test("Header hovered when scrolled", () => {
        const spy = vi.fn();
        render(_jsx(DataGrid, { ...basicProps, groupHeaderHeight: 32, enableGroups: true, cellYOffset: 10, onItemHovered: spy }));
        const el = screen.getByTestId(dataGridCanvasId);
        fireEvent.mouseMove(el, {
            clientX: 350, // Col C
            clientY: 46, // Header
    ...
    test("Group header hovered", () => {
        const spy = vi.fn();
        render(_jsx(DataGrid, { ...basicProps, onItemHovered: spy, enableGroups: true, groupHeaderHeight: 28 }));
        const el = screen.getByTestId(dataGridCanvasId);
        fireEvent.mouseMove(el, {
            clientX: 350, // Col C
            clientY: 14, // Header
    ...
    test("Simple damage", () => {
        ...
        render(_jsx(DataGrid, { ref: ref, ...basicProps, getCellContent: spy, enableGroups: true, groupHeaderHeight: 28 }));
        spy.mockClear();
    ...
    test("Freeze column simple check", () => {
        const spy = vi.fn();
        render(_jsx(DataGrid, { ...basicProps, freezeColumns: 1, cellXOffset: 3, onMouseUp: spy }));
        fireEvent.mouseDown(screen.getByTestId(dataGridCanvasId), {
            clientX: 50, // Col A
            clientY: 36 + 32 * 5 + 16, // Row 5 (0 indexed)
        });
