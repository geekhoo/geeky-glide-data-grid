...
import { CompactSelection, DataEditor, GridCellKind, isSizedGridColumn, } from "../src/index.js";
import { assert } from "../src/common/support.js";
import { vi, expect, describe, test, beforeEach, afterEach } from "vitest";
import { EventedDataEditor, basicProps, prep, sendClick, sendTouchClick, getCellCenterPositionForDefaultGrid, Context, standardBeforeEach, standardAfterEach, } from "./test-utils.js";
describe("data-editor", () => {
    ...
    test("Focus a11y cell", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep(false);
    ...
    test("Click a11y cell", async () => {
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps }), {
            wrapper: Context,
        });
        prep(false);
        const a11ycell = screen.getByTestId("glide-cell-0-5");
    ...
    test("emits contextmenu for cell", async () => {
        ...
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onCellContextMenu: spy, onGridSelectionChange: spySelection }), {
            wrapper: Context,
        });
        const scroller = prep();
        ...
        expect(spySelection).toHaveBeenCalledWith(expect.objectContaining({
    ...
    test("emits contextmenu for row marker", async () => {
        ...
        const spySelection = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, rowMarkers: "both", onCellContextMenu: spy, onGridSelectionChange: spySelection }), {
            wrapper: Context,
        });
        const scroller = prep();
        assert(scroller !== null);
    ...
    test("emits contextmenu for cell but does not change selection if already selected - rows", async () => {
        ...
        const spySelection = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, gridSelection: {
                columns: CompactSelection.empty(),
                rows: CompactSelection.fromSingleSelection(1),
            }, onCellContextMenu: spy, onGridSelectionChange: spySelection }), {
            wrapper: Context,
        });
        const scroller = prep();
        assert(scroller !== null);
    ...
    test("Emits cell click", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onCellClicked: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Emits activated event on double click", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onCellActivated: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    describe("cellActivationBehavior", () => {
        test("double-click in time", async () => {
            const spy = vi.fn();
            vi.useFakeTimers();
            render(_jsx(DataEditor, { ...basicProps, onCellActivated: spy, cellActivationBehavior: "double-click" }), {
                wrapper: Context,
            });
            prep(false);
            const canvas = screen.getByTestId("data-grid-canvas");
        ...
        test("second-click", async () => {
            const spy = vi.fn();
            vi.useFakeTimers();
            render(_jsx(DataEditor, { ...basicProps, onCellActivated: spy, cellActivationBehavior: "second-click" }), {
                wrapper: Context,
            });
            prep(false);
            const canvas = screen.getByTestId("data-grid-canvas");
        ...
        test("single-click", async () => {
            const spy = vi.fn();
            vi.useFakeTimers();
            render(_jsx(DataEditor, { ...basicProps, onCellActivated: spy, cellActivationBehavior: "single-click" }), {
                wrapper: Context,
            });
            prep(false);
            const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Toggle boolean with Enter key", async () => {
        ...
        const editSpy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onCellActivated: spy, onCellEdited: editSpy }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("keyDown and keyUp events include the cell location", async () => {
        ...
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onKeyDown: keyDown, onKeyUp: keyUp }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Doesn't emit header click if mouseDown happened in a different cell", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onHeaderClicked: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Uneven rows cell click", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onCellClicked: spy, rowHeight: r => (r % 2 === 0 ? 32 : 64) }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Emits finished editing", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onFinishedEditing: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
        ...
        expect(spy).toBeCalledWith({ allowOverlay: true, allowWrapping: true, data: "j", displayData: "1, 1", kind: "text" }, [0, 1]);
    });
    test("Does not edit when validation fails", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onCellEdited: spy, validateCell: () => false }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Does emit header click on row marker column", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, rowMarkers: "both", onHeaderClicked: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Group header sections", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, getGroupDetails: g => ({
                name: g,
                icon: "headerCode",
            }), columns: basicProps.columns.map(c => ({ ...c, group: "A" })), onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Rename group header shows", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, columns: basicProps.columns.map(c => ({ ...c, group: c.title })), onGroupHeaderRenamed: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Emits header menu click", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, columns: basicProps.columns.map(c => ({ ...c, hasMenu: true })), onHeaderMenuClick: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Emits group header clicked on touch", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, columns: basicProps.columns.map(c => ({ ...c, group: "Main" })), rowMarkers: "both", onGroupHeaderClicked: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Emits item hover on correct location", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, rowMarkers: "both", onItemHovered: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Emits mouse move on correct location", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, rowMarkers: "both", onMouseMove: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Delete cell", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onDelete: spy, gridSelection: {
                columns: CompactSelection.empty(),
                rows: CompactSelection.empty(),
                current: {
                    cell: [2, 2],
    ...
    test("Delete cell callback result", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onDelete: sel => sel, onCellEdited: spy, gridSelection: {
                columns: CompactSelection.empty(),
                rows: CompactSelection.empty(),
                current: {
                    cell: [2, 2],
    ...
    test("Delete custom", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, getCellContent: () => ({
                ...
                {
                    draw: () => undefined,
                    isMatch: (c) => c.kind === GridCellKind.Custom,
                    kind: GridCellKind.Custom,
                    onDelete: spy,
                },
            ], onDelete: sel => sel, gridSelection: {
                columns: CompactSelection.empty(),
                rows: CompactSelection.empty(),
                current: {
                    cell: [2, 2],
    ...
    test("Open and close overlay", async () => {
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Open overlay with keypress when prior is disabled", async () => {
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, getCellContent: cell => {
    ...
    test("Send edit", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, onCellEdited: spy }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Directly toggle booleans", async () => {
        ...
        render(_jsx(DataEditor, { ...basicProps, onCellEdited: spy, ref: ref }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Ref getBounds", async () => {
        ...
        vi.useFakeTimers();
        const ref = React.createRef();
        render(_jsx(DataEditor, { ...basicProps, onCellEdited: spy, ref: ref, rowMarkers: "both" }), {
            wrapper: Context,
        });
        prep(false);
        act(() => {
    ...
    test("Freeze area reported", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, freezeTrailingRows: 2, freezeColumns: 3, onVisibleRegionChanged: spy }), {
            wrapper: Context,
        });
        prep();
        expect(spy).toBeCalledWith(expect.objectContaining({
    ...
    test("Search close", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, showSearch: true, onSearchClose: spy }), {
            wrapper: Context,
        });
        prep(false);
    ...
    test("Copy/paste", async () => {
        const spy = vi.fn();
        const pasteSpy = vi.fn((_target, _values) => true);
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onGridSelectionChange: spy, onPaste: (...args) => pasteSpy(...args) }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
        ...
        expect(pasteSpy).toBeCalledWith([1, 3], [
    ...
    test("Paste out of range does not crash", async () => {
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onPaste: true }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Cut cell", async () => {
        ...
        const editSpy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onGridSelectionChange: spy, onCellsEdited: editSpy }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Paste custom cell does not crash", async () => {
        ...
        const spy = vi.fn();
        render(_jsx(EventedDataEditor, { ...basicProps, getCellContent: alwaysCustomCell, customRenderers: [
                {
                    kind: GridCellKind.Custom,
                    draw: () => true,
                    onPaste: spy,
    ...
    test("onCellsEdited blocks onCellEdited", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onCellEdited: spy, onCellsEdited: () => true }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Copy/paste with simple getCellsForSelection", async () => {
        ...
        const pasteSpy = vi.fn((_target, _values) => true);
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, getCellsForSelection: true, onGridSelectionChange: spy, onPaste: (...args) => pasteSpy(...args) }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Copy rows", async () => {
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, rowMarkers: "both", gridSelection: {
                current: undefined,
        ...
        fireEvent.copy(window);
        await act(() => new Promise(r => window.setTimeout(r, 10)));
        expect(navigator.clipboard.writeText).toBeCalledWith("Data: 0, 3\t1, 3\t2, 3\t3\tFoobar\t************\tFoobar\t\tשלום 8, 3\t# Header: 9, 3\thttps://example.com/10/3");
    });
    test("Copy cols", async () => {
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, gridSelection: {
    ...
    test("Hover header does not fetch invalid cell", async () => {
        ...
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps, rowMarkers: "both", getCellContent: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Blit does not crash vertical scroll", async () => {
        ...
        await act(() => new Promise(r => window.setTimeout(r, 100)));
        if (scroller !== null) {
            vi.spyOn(scroller, "scrollWidth", "get").mockImplementation(() => basicProps.columns.map(c => (isSizedGridColumn(c) ? c.width : 150)).reduce((pv, cv) => pv + cv, 0));
    ...
    test("Blit does not crash horizontal scroll", async () => {
        vi.useFakeTimers();
        render(_jsx(DataEditor, { highlightRegions: [
                {
                    color: "#12345623",
                    range: {
                        x: 2,
    ...
    test("New row", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onRowAppended: spy, trailingRowOptions: {
                hint: "New Row",
                sticky: true,
            } }), {
            wrapper: Context,
    ...
    test("Click row marker", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onGridSelectionChange: spy, rowMarkers: "both" }), {
            wrapper: Context,
        });
        prep();
        vi.useFakeTimers();
    ...
    test("Shift click row marker - no multi-select", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, rowSelect: "single", onGridSelectionChange: spy, rowMarkers: "both" }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Fill down", async () => {
        ...
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, keybindings: {
                downFill: true,
            }, onCellsEdited: multiSpy, onCellEdited: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Fill right", async () => {
        ...
        const multiSpy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, keybindings: { rightFill: true }, onCellEdited: spy, onCellsEdited: multiSpy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Delete range", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onCellEdited: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Click out of bounds", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, columns: basicProps.columns.slice(0, 2), onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("DND Columns", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onColumnMoved: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Drag reorder row", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, rowMarkers: "number", onRowMoved: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Select all", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, experimental: { renderStrategy: "double-buffer" }, rowMarkers: "both", onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Draggable", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, rowMarkers: "both", onDragStart: e => {
                spy(e);
    ...
    test("Click cell does not double-emit selectedrows/columns", async () => {
        ...
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onGridSelectionChange: gridSelectionSpy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
        ...
        expect(gridSelectionSpy).toBeCalledWith(expect.objectContaining({
    ...
    test("Span expansion", async () => {
        ...
        render(_jsx(EventedDataEditor, { ...basicProps, getCellContent: getCellContent, getCellsForSelection: true, onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Imperative Handle works", async () => {
        ...
        vi.useFakeTimers();
        const ref = React.createRef();
        render(_jsx(EventedDataEditor, { ref: ref, ...basicProps, onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep();
        act(() => {
    ...
    test("Imperative scrollTo false fire", async () => {
        vi.useFakeTimers();
        const ref = React.createRef();
        render(_jsx(EventedDataEditor, { ref: ref, ...basicProps, rows: 10_000 }), {
            wrapper: Context,
        });
        prep(false);
        act(() => {
    ...
    test("Imperative damage gets right cell", async () => {
        ...
        vi.useFakeTimers();
        const ref = React.createRef();
        render(_jsx(EventedDataEditor, { ref: ref, ...basicProps, rowMarkers: "number", getCellContent: spy }), {
            wrapper: Context,
        });
        prep();
        spy.mockClear();
    ...
    test("Ctrl Arrow keys", async () => {
        ...
        expect(spy).toBeCalledWith(expect.objectContaining({ current: expect.objectContaining({ cell: [cols - 1, 999] }) }));
        spy.mockClear();
        fireEvent.keyDown(canvas, {
            key: "ArrowUp",
            ctrlKey: true,
        });
        expect(spy).toBeCalledWith(expect.objectContaining({ current: expect.objectContaining({ cell: [cols - 1, 0] }) }));
        spy.mockClear();
        fireEvent.keyDown(canvas, {
            key: "ArrowLeft",
            ctrlKey: true,
    ...
    test("Select range with mouse going out of bounds", async () => {
        ...
        render(_jsx(EventedDataEditor, { ...basicProps, columns: columns, onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Select all keybind", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, keybindings: { selectAll: true }, onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Select column with blending", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, rowSelectionBlending: "mixed", columnSelectionBlending: "mixed", rangeSelectionBlending: "mixed", onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Dragging header disables vertical autoscroll", async () => {
        const spy = Element.prototype.scrollBy;
        spy.mockClear();
        vi.useFakeTimers();
        render(_jsx(DataEditor, { ...basicProps }), {
            wrapper: Context,
    ...
    test("Use fill handle", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onCellEdited: spy, fillHandle: true }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("onFillPattern", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onFillPattern: spy, fillHandle: true }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Use fill handle into blank", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, rows: 3, onCellEdited: spy, fillHandle: true }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Use fill handle into trailing row", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, rows: 3, onCellEdited: spy, fillHandle: true, onRowAppended: () => undefined, trailingRowOptions: {
                sticky: true,
            } }), {
            wrapper: Context,
        });
    ...
    test("Close overlay with enter key", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { experimental: {
                strict: true,
            }, ...basicProps, onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Clear selection when suddenly out of range", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        const { rerender } = render(_jsx(EventedDataEditor, { ...basicProps, rows: 10, onGridSelectionChange: spy }), {
            wrapper: Context,
        });
        prep(false);
        const canvas = screen.getByTestId("data-grid-canvas");
        ...
        spy.mockClear();
        rerender(_jsx(EventedDataEditor, { ...basicProps, rows: 1, onGridSelectionChange: spy }));
        act(() => {
            vi.runAllTimers();
        });
        expect(spy).toBeCalledWith({
    ...
    test("Enter key does not trigger disallowed row fetch", async () => {
        const spy = vi.fn(basicProps.getCellContent);
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, rows: 2, getCellContent: spy, onRowAppended: vi.fn(), trailingRowOptions: {
                sticky: true,
                tint: true,
            } }), {
            wrapper: Context,
