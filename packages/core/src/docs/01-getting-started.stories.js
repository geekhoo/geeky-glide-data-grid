...
The columns of the data grid may contain many options, including icons, menus, theme overrides, however at their most basic they only require a \`title\` and an \`id\`. The id is technically optional but it is best not to omit it.` }), _jsx(Highlight, { children: `
...
Finally the data grid requires a cell fetch callback. This callback should be memoized using \`React.useCallback\` or be a static function.` }), _jsx(Highlight, { children: `
...
That is all the basic requirements put together.` }), _jsx(Highlight, { children: `
return <DataEditor getCellContent={getContent} columns={columns} rows={data.length} />;
` }), _jsx(Wrapper, { height: 200, children: _jsx(DataEditor, { getCellContent: getContent, columns: columns, rows: data.length }) }), _jsx(Marked, { children: `
# Going further
...
Here are a few to play with.` }), _jsxs("label", { style: { display: "block" }, children: [_jsx("input", { type: "checkbox", checked: rowMarkers, onChange: e => setRowMarkers(e.target.checked) }), " Row Markers"] }), _jsxs("label", { style: { display: "block" }, children: [_jsx("input", { type: "checkbox", checked: smoothScroll, onChange: e => setSmoothScroll(e.target.checked) }), " ", "Smooth Scroll"] }), _jsxs("label", { style: { display: "block" }, children: [_jsx("input", { type: "checkbox", checked: verticalBorder, onChange: e => setVerticalBorder(e.target.checked) }), " ", "Vertical Borders"] }), _jsx(Wrapper, { height: 200, children: _jsx(DataEditor, { getCellContent: getContent, verticalBorder: verticalBorder, smoothScrollX: smoothScroll, smoothScrollY: smoothScroll, rowMarkers: rowMarkers ? "both" : "none", columns: columns, rows: data.length }) })] }));
};
