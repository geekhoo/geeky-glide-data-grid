...
import { remapForDnDState, drawLastUpdateUnderlay, } from "../src/internal/data-grid/render/data-grid-lib.js";
...
function makeCol(title, sourceIndex, sticky, width) {
    return {
        title,
        sourceIndex,
        sticky,
...
describe("drawImage", () => {
    ...
    test("renders with basic input", () => {
        ...
        expect(mockCtx.drawImage).toHaveBeenCalledWith(mockImage, expect.any(Number), expect.any(Number), expect.any(Number), expect.any(Number));
    });
    ...
    test("renders aligned to the right", () => {
        ...
        expect(mockCtx.drawImage).toHaveBeenCalledWith(mockImage, expectedX, expect.any(Number), expect.any(Number), expect.any(Number));
    });
...
describe("drawWithLastUpdate", () => {
    ...
    it("should do nothing if lastUpdate is undefined", () => {
        const result = drawLastUpdateUnderlay({
            ...
            imageLoader: {},
            spriteManager: {},
        }, undefined, 1000, undefined, false, false);
    ...
    it("should not animate if progress is >= animTime", () => {
        ...
        const result = drawLastUpdateUnderlay({
            ...
            imageLoader: {},
            spriteManager: {},
        }, lastUpdate, frameTime, undefined, false, false);
        expect(mockCtx.fillStyle).toBe("");
        expect(result).toBe(false);
    });
    it("should animate if progress is < animTime", () => {
        ...
        expect(mockCtx.fillRect).toHaveBeenCalledWith(mockRect.x + 1, mockRect.y + 1, mockRect.width - 1, mockRect.height - 1);
    ...
    it("should update lastPrep's fillStyle if defined", () => {
        ...
        drawLastUpdateUnderlay({
            ...
            imageLoader: {},
            spriteManager: {},
        }, lastUpdate, frameTime, mockLastPrep, false, false);
