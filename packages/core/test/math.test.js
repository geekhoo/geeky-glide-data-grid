...
describe("getClosestRect", () => {
    const testRect = { x: 10, y: 10, width: 5, height: 5 };
    ...
    it("should work with single cell starting rect", () => {
        expect(getClosestRect({
            x: 10,
            y: 10,
            width: 1,
            height: 1,
        }, 20, 10, "orthogonal")).to.deep.equal({ x: 11, y: 10, width: 10, height: 1 });
    });
});
...
describe("splitRectIntoRegions", () => {
    const splitIndices = [2, 2, 8, 8];
    const width = 100;
    const height = 100;
    const splitLocations = [20, 20, 80, 80];
