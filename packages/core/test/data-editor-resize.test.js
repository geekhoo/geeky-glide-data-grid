...
describe("data-editor", () => {
    ...
    test("Resize Column", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onColumnMoved: spy, onColumnResize: spy }), {
            wrapper: Context,
        });
        prep();
    ...
    test("Auto Resize Column", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onColumnResize: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Auto Resize Column Ref", async () => {
        ...
        render(_jsx(EventedDataEditor, { ...basicProps, ref: ref, onColumnResize: spy }), {
            wrapper: Context,
        });
        prep();
    ...
    test("Resize Column End Called", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onColumnResize: vi.fn(), onColumnResizeEnd: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
    ...
    test("Resize column end called correct number of times", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, onColumnResize: vi.fn(), onColumnResizeEnd: spy, gridSelection: {
    ...
    test("Resize Last Column", async () => {
        const spy = vi.fn();
        vi.useFakeTimers();
        render(_jsx(EventedDataEditor, { ...basicProps, columns: basicProps.columns.slice(0, 2), onColumnMoved: spy, onColumnResize: spy }), {
            wrapper: Context,
        });
        prep();
        const canvas = screen.getByTestId("data-grid-canvas");
