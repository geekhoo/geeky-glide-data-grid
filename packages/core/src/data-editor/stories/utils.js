...
export const BeautifulWrapper = p => {
    ...
    return (_jsxs(BeautifulStyle, { className: className + (browserIsFirefox.value ? " firefox" : ""), children: [_jsx("h1", { children: title }), description, _jsx("div", { style: { scale }, className: "sizer", children: _jsx("div", { className: "sizer-clip", ref: ref, children: _jsx("div", { style: {
                            position: "relative",
                            width: width ?? 100,
                            height: height ?? 100,
                        }, children: children }) }) })] }));
...
export function useMockDataGenerator(numCols, readonly = true, group = false) {
    ...
    const setCellValue = React.useCallback(([col, row], val) => {
...
export function useAllMockedKinds() {
    ...
    const setCellValue = React.useCallback(([col, row], val, noDisplay, forceUpdate) => {
        let current = cache.current.get(col, row);
        if (current === undefined) {
            current = colsMap[col].getContent();
        }
