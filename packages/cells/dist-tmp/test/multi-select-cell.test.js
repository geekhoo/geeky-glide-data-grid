...
import { getByText, queryByText, getByRole, findByText, fireEvent, render, cleanup, queryByRole, } from "@testing-library/react";
import { vi, expect, describe, it, afterEach } from "vitest";
import { GridCellKind } from "@glideapps/glide-data-grid";
import renderer, { prepareOptions, resolveValues } from "../src/cells/multi-select-cell.js";
...
export function hasOption(container, optionText) {
    const inputElement = getByRole(container, "combobox");
    fireEvent.keyDown(inputElement, keyDownEvent);
    const listBox = getByRole(container, "listbox");
...
describe("Multi Select Editor", () => {
    ...
    it("renders into the dom with correct value", async () => {
        ...
        const result = render(_jsx(Editor, { isHighlighted: false, value: getMockCell(), onChange: mockCellOnChange }));
    ...
    it("allows to select values", async () => {
        ...
        const mockCellOnChange = vi.fn();
        const result = render(_jsx(Editor, { isHighlighted: false, value: mockCell, onChange: mockCellOnChange }));
        // Check if the element is actually there
        const cellEditor = result.getByTestId("multi-select-cell");
        expect(cellEditor).toBeDefined();
