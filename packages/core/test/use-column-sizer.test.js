...
import { GridCellKind } from "../src/index.js";
...
const A_BUNCH_OF_COLUMNS_THAT_ALREADY_HAVE_SIZES_WE_DONT_WANT_TO_MEASURE_THESE = [
    {
        title: "A",
        width: 120,
    },
...
function buildCellsForSelectionGetter(dataBuilder) {
    return (selection) => {
        const result = [];
...
const getShortCellsForSelection = vi.fn(buildCellsForSelectionGetter((x, y) => `column ${x} row ${y}`));
const getLongCellsForSelection = vi.fn(buildCellsForSelectionGetter((x, y) => `This cell is in column number ${x} and row number ${y}`));
...
const getCellRenderer = cell => {
    if (cell.kind === GridCellKind.Custom)
        return undefined;
    return AllCellRenderers.find(x => x.kind === cell.kind);
};
describe("use-column-sizer", () => {
    ...
    it("Measures a simple cell", async () => {
        const { result } = renderHook(() => useColumnSizer(COLUMNS, 1000, getShortCellsForSelection, 400, 20, 500, mergeAndRealizeTheme(theme), getCellRenderer, abortController));
    ...
    it("Measures a custom cell", async () => {
        const { result } = renderHook(() => useColumnSizer(SINGLE_COLUMN, 1000, (selection) => {
            const r = [];
            for (let y = selection.y; y < selection.y + selection.height; y++) {
                const row = [];
                for (let x = selection.x; x < selection.x + selection.width; x++) {
            ...
            return r;
        }, 400, 20, 500, mergeAndRealizeTheme(theme), () => {
            return {
                draw: () => true,
                kind: GridCellKind.Custom,
                measure: () => 212,
    ...
    it("Measures column width based on title if longer than data", async () => {
        ...
        const { result } = renderHook(() => useColumnSizer(columns, 500, getShortCellsForSelection, 400, 20, 500, mergeAndRealizeTheme(theme, { cellHorizontalPadding: 12 }), getCellRenderer, abortController));
    ...
    it("Measures the last row", async () => {
        ...
        renderHook(() => useColumnSizer(COLUMNS, 1000, getShortCellsForSelection, 400, 20, 500, mergeAndRealizeTheme(theme), getCellRenderer, abortController));
        expect(getShortCellsForSelection).toBeCalledTimes(2);
        expect(getShortCellsForSelection).toHaveBeenNthCalledWith(1, {
            x: 0,
            y: 0,
            width: 2,
            height: 9,
    ...
    it("Measures new columns when they arrive, doesn't re-measure existing ones", async () => {
        const { result, rerender } = renderHook(({ getCellsForSelection, columns }) => useColumnSizer(columns, 1000, getCellsForSelection, 400, 20, 500, mergeAndRealizeTheme(theme), getCellRenderer, abortController), {
            initialProps: {
    ...
    it("Returns the default sizes if getCellsForSelection is not provided", async () => {
        const { result } = renderHook(() => useColumnSizer(COLUMNS, 1000, undefined, 400, 20, 500, mergeAndRealizeTheme(theme), getCellRenderer, abortController));
        const columnA = result.current.sizedColumns.find(col => col.title === "A");
        const columnB = result.current.sizedColumns.find(col => col.title === "B");
        expect(columnA).toBeDefined();
        expect(columnB).toBeDefined();
    ...
    it("Does not measure anything if every column is sized", async () => {
        const { result } = renderHook(() => useColumnSizer(A_BUNCH_OF_COLUMNS_THAT_ALREADY_HAVE_SIZES_WE_DONT_WANT_TO_MEASURE_THESE, 1000, undefined, 400, 50, 500, mergeAndRealizeTheme(theme), getCellRenderer, abortController));
        expect(result.current.sizedColumns).toBe(A_BUNCH_OF_COLUMNS_THAT_ALREADY_HAVE_SIZES_WE_DONT_WANT_TO_MEASURE_THESE);
    });
    it("Removes the canvas from the DOM when unmounted", async () => {
        // eslint-disable-next-line sonarjs/no-identical-functions
        const { unmount } = renderHook(() => useColumnSizer(COLUMNS, 1000, getShortCellsForSelection, 400, 20, 500, mergeAndRealizeTheme(theme), getCellRenderer, abortController));
    ...
    it("Distributes extra width among columns if total column width is less than client width", async () => {
        ...
        const { result } = renderHook(() => useColumnSizer(SMALL_COLUMNS, 500, getShortCellsForSelection, 400, 20, 500, mergeAndRealizeTheme(theme), getCellRenderer, abortController));
        const columnA = result.current.sizedColumns.find(col => col.title === "A");
        const columnB = result.current.sizedColumns.find(col => col.title === "B");
